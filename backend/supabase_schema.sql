...
-- users
-- No direct client access. All operations go through the Flask backend.
...
-- user_otps
-- Completely internal — no client-side access ever.
...
-- sessions
-- Internal token store — no client-side access.
...
-- menu_items
-- Public SELECT is intentionally allowed for available items only.
-- All writes are blocked for anon and authenticated — only service_role writes.
...
-- orders
-- No direct client access. Session-based auth is handled in Flask middleware.
...
-- Run this block in the SQL Editor after the schema is set up.
-- =============================================
